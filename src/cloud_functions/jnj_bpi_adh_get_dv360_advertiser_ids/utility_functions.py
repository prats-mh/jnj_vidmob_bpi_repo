...
def get_dv360_ads_data_links(service):
    try:
        response = (
            ...
            .adsDataLinks()
            .list(
                parent=con_prop["customer_name"],
                filter=con_prop["datalinks_filter_clause"],
                pageSize=500,
            )
            .execute()
        )
...
def send_start_transient_query_request(data_link, service):
    ...
    spec = {
        "adsDataCustomerId": data_link["adsDataCustomerId"],
        "startDate": {
            "year": date_range["start_date_year"],
            "month": date_range["start_date_month"],
            "day": date_range["start_date_day"],
        },
        "endDate": {
            "year": date_range["end_date_year"],
            "month": date_range["end_date_month"],
            "day": date_range["end_date_day"],
        },
        "parameterValues": {
            query_parameter_name: {"arrayValue": {"values": param_values}}
        },
    }
    ...
    body = {
        "query": query_text,
        "spec": spec,
        "destTable": f"jjt-consumerdatalake-bigquery.{con_prop['bq_dataset_name']}.{con_prop['bq_table_name_template']}_{data_link['adsDataCustomerId']}",
    }
    ...
    try:
        response = (
            service.customers()
            .analysisQueries()
            .startTransient(parent=con_prop["customer_name"], body=body)
            .execute()
        )
        return response
    except Exception as e:
...
def poll_operation(operation_name, service):
    while True:
        operation_state = (
            service.operations()
            .wait(name=operation_name, body={"timeout": "120s"})
            .execute()
        )
...
def send_pubsub_msg(bq_table_list):
    ...
    with open("pubsub_message.txt", "wb") as f:
...
def curate_ads_data_links_response(response):
    ...
    for link in response["links"]:
        ...
        if adsDataCustomerId not in result:
            result[adsDataCustomerId] = {"adsDataCustomerId": adsDataCustomerId}

        if link_type == "GMP_DV360_PARTNER":
            if "partner_ids" not in result[adsDataCustomerId]:
                result[adsDataCustomerId]["partner_ids"] = [{"value": entityid}]
            else:
                result[adsDataCustomerId]["partner_ids"].append({"value": entityid})
        elif link_type == "GMP_DV360_ADVERTISER":
            if "advertiser_ids" not in result[adsDataCustomerId]:
                result[adsDataCustomerId]["advertiser_ids"] = [{"value": entityid}]
            else:
                result[adsDataCustomerId]["advertiser_ids"].append({"value": entityid})

    final = list(result.values())
    both_ids = [x for x in final if "partner_ids" in x and "advertiser_ids" in x]
    final_mins_both_ids = [
        x for x in final if not ("partner_ids" in x and "advertiser_ids" in x)
    ]
    for x in both_ids:
        partner_ids = {
            "adsDataCustomerId": x["adsDataCustomerId"],
            "partner_ids": x["partner_ids"],
        }
        advertiser_ids = {
            "adsDataCustomerId": x["adsDataCustomerId"],
            "advertiser_ids": x["advertiser_ids"],
        }
...
def date_range_dict():
    ...
    first_date_of_last_2_months = last_date_of_last_2_months.replace(day=1)  # noqa

    start_date = date(2022, 1, 1)  # first_date_of_last_2_months
    end_date = date(2023, 1, 3)  # last_date_of_last_month
