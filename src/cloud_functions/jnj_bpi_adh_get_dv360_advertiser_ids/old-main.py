...
from utility_functions import get_dv360_ads_data_links, send_start_transient_query_request, poll_operation, bq_table_list, send_pubsub_msg, write_json_file, read_json_file
import json
# import time
...
creds = Credentials.from_service_account_info(
    config_properties['adh_svc_acct_secret']).with_scopes(SCOPES)

adh_service = build('AdsDataHub', 'v1', credentials=creds,
                    developerKey=config_properties['adh_api_key'], discoveryServiceUrl=DISCOVERY_URL)
...
pub_res = send_pubsub_msg('tl_list') #table_lists
